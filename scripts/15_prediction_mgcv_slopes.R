...
predict_gridded_level_5_slopes <- function(m,i,l,j,k){ # this order, together with the function below, 
  # ensures that the list goes k (model), j (crop), l (time period), i (GCM), m (imputed)
  gammit::predict_gamm(crop_models_slopes[[k]][[j]][[m]],
...
prediction_gridded_level_5_slopes <- lapply(1:2, function(k){ # k = model spec
  lapply(1:4, function(j, k){ # j = crop 
    lapply(1:4, function(l, j, k){ # l = time period 
      lapply(12, function(i, l , j, k){ # i = GCM
        lapply(1:5, predict_gridded_level_5_slopes, i, l, j, k) # m = imputed
      }, l, j, k)
    }, j, k) 
  }, k)
}) 
...
saveRDS(prediction_gridded_level_5_slopes, here("results", "predictions", "prediction_gridded_level_5_slopes.RData")) # saved output from GCM=1
prediction_gridded_level_5_slopes <- readRDS(here("results", "predictions", "prediction_gridded_level_5_slopes.RData")) 

# function to rasterise all predictions and plot  --------


add_coords_to_predictions_mgcv_slopes <- function(m,i,l,j,k){
  ...
  predictions <- data.frame(lon = coords_countries$lon, lat = coords_countries$lat) %>%
    left_join(dplyr::select(
      prediction_gridded_level_5_slopes[[k]][[j]][[l]][[i]][[m]], #k (model), j (crop), l (time period), i (GCM), m (imputed)
      prediction.fit, lon, lat), 
      by = c("lon", "lat"))
}

predictions_mgcv_coords_slopes  <- lapply(1:2, function(k){ # k = model spec
  lapply(1:4, function(j, k){ # j = crop 
    lapply(1:4, function(l, j, k){ # l = time period 
      lapply(1, function(i, l , j, k){ # i = GCM
        lapply(1:5, add_coords_to_predictions_mgcv_slopes, i, l, j, k) # m = imputed
      }, l, j, k)
    }, j, k) 
  }, k)
}) 

saveRDS(predictions_mgcv_coords_slopes, here("results", "predictions", "predictions_mgcv_coords_slopes.RData"))

predictions_mgcv_raster_slopes <- lapply(1:2, function(k){ # k = model spec
  lapply(1:4, function(j, k){ # j = crop 
    lapply(1:4, function(l, j, k){ # l = time period 
      lapply(1, function(i, l , j, k){ # i = GCM
        lapply(1:5, function(m,i,l,j,k){
          rasterFromXYZ(predictions_mgcv_coords_slopes[[k]][[j]][[l]][[i]][[m]])
        }, i, l, j, k) # m = imputed
      }, l, j, k)
    }, j, k) 
  }, k)
...
rasterVis::levelplot(predictions_mgcv_raster[[1]][[2]][[4]][[1]][[5]],  # [[k]][[j]][[l]][[i]][[m]]
                     at=seq(-100,100),
...
predictions_country_mgcv_slopes <- lapply(1, function(k){ # k = model spec
  lapply(4, function(j, k){ # j = crop 
    lapply(4, function(l, j, k){ # l = time period 
      lapply(1, function(i, l , j, k){ # i = GCM
        lapply(5, function(m,i,l,j,k){ # m = imputed
          
          prediction <- exactextractr::exact_extract(predictions_mgcv_raster_slopes[[k]][[j]][[l]][[i]][[m]], 
                                                     worldmap_clean, 
                                                     'weighted_mean', 
...
predictions_country_mgcv_slopes[[1]][[1]][[1]][[1]][[1]] %>% summarise(mean=mean(prediction,na.rm=TRUE))
