...
multiply_imp_for_plot_gam <- function(data){
...
fit_lines <- function(fit, data){
  ...
  lapply(1:4, function(i){ # crop subscripts
    
      tibble(.imp=1:5) %>% # m subscripts
    mutate(p=purrr::map(.imp, ~ 
                                ggpredict(fit[[i]]$analyses[[.]],
...
rbind_grouped_lines <- function(grouped_lines){
  ...
  rbindlist(grouped_lines, idcol="crop") %>% 
...
save_plots_reponse_functions <- function(plots, path){
