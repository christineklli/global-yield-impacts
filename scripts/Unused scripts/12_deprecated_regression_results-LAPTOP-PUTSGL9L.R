# should I generate results for all models or only for the cross validated ones?
# create code infrastructure to run for all models - go back and cross validate later
...
summary_fit_glmm <- lapply(1:5, function(k){ # formula = k - model 3 and 4 most interesting
  lapply(1:4, function(j, k){ # crop = j
    lapply(1:5, function(i,j,k){
...
predicted_yield <-  ggpredict(crop_gamm[[1]][[1]][[1]],
                              # terms = c("Temp.Change", "Baseline_tmp_weighted [median]"), 
                              terms = c("Temp.Change", "Precipitation.change"),
